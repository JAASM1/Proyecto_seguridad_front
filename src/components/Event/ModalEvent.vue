    ...
    <div class="space-y-6">
      <div v-if="acceptedGuests.length > 0">
        <h3 class="text-lg font-semibold text-primary mb-3">
          Aceptados ({{ acceptedGuests.length }})
        </h3>
        <div class="space-y-4">
          <div
      ...
      <div v-if="canceledGuests.length > 0">
        <h3 class="text-lg font-semibold text-danger mb-3">
          Cancelados ({{ canceledGuests.length }})
        </h3>
        <div class="space-y-4">
          <div
      ...
      <div
        v-if="!acceptedGuests.length && !canceledGuests.length"
...
const deleteGuest = async (guest: Guest) => {
  try {
    await axios.put(
      `${import.meta.env.VITE_API_URL}/Invitation/RemoveUser/${guest.idInvitation}`
    );
    ...
    toast.add({
      severity: 'success',
      summary: 'Invitado removido',
      detail: `${guest.name} fue desasociado de la invitación`,
      life: 3000
    });
    emit('guest-updated');
  } catch (error) {
    handleError(error, 'remover invitado');
  }
};
